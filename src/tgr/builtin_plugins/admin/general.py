...
PLUGIN_META = {
    "name": "general",
    "display_name": "系统帮助与状态",
    "version": "1.0.0",
    "description": "帮助、状态、日志、版本与后台任务查看。",
    "mode": "admin",
    "api_version": "1",
}
...
def register(registry) -> None:
    @registry.command("help", summary="按已注册插件展示帮助", usage="help [插件名]", category="general", plugin="general")
    async def help_cmd(ctx):
        filter_text = (ctx.args or "").strip()
        await ctx.app.reply_panel(
            ctx.event,
            render_help_panel(ctx.app.config.cmd_prefix, ctx.app.registry.unique_specs(), ctx.app.plugin_manager.admin_states, filter_text),
            auto_delete=0,
        )

    @registry.command("ping", summary="检查前台命令热路径是否在线", usage="ping", category="general", plugin="general")
    async def ping_cmd(ctx):
        await ctx.app.reply_panel(ctx.event, ctx.app.render_ping_message(), auto_delete=0)

    @registry.command("status", summary="查看系统总体状态", usage="status", category="general", plugin="general")
    async def status_cmd(ctx):
        await ctx.app.reply_panel(ctx.event, ctx.app.render_status_message(), auto_delete=0)

    @registry.command("config", summary="查看关键配置", usage="config", category="general", plugin="general")
    async def config_cmd(ctx):
        await ctx.app.reply_panel(ctx.event, ctx.app.render_config_message(), auto_delete=0)

    @registry.command("version", summary="查看版本和架构", usage="version", category="general", plugin="general")
    async def version_cmd(ctx):
        await ctx.app.reply_panel(ctx.event, ctx.app.render_version_message(), auto_delete=0)

    @registry.command("jobs", summary="查看后台任务队列", usage="jobs", category="general", plugin="general")
    async def jobs_cmd(ctx):
        await ctx.app.reply_panel(ctx.event, ctx.app.render_jobs_message(), auto_delete=0)

    @registry.command("log", summary="查看最近系统日志", usage="log [important|normal|all] [20]", category="general", plugin="general")
