...
class AdminExecutors:
    ...
    async def _execute_reload_core(self, job: AdminJob) -> JobResult:
        ...
        proc = await asyncio.create_subprocess_exec("systemctl", "kill", "-s", "USR1", f"{self.config.service_name_prefix}-core", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.STDOUT)
        stdout, _ = await proc.communicate()
        output = (stdout or b"").decode("utf-8", errors="replace").strip()
        if proc.returncode == 0:
            return JobResult(status="done", summary="Core 已触发重载", detail=detail or output or reason, log_action="CORE_RELOAD")
        fallback = await asyncio.create_subprocess_exec("bash", "-lc", "pkill -USR1 -f 'src/radar_core.py'", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.STDOUT)
        ...
        return JobResult(status="failed", summary="Core 重载失败", detail=output or fb_text or "无法向 Core 发送重载信号", log_action="CORE_RELOAD", log_level="ERROR")
