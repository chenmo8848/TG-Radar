...
LOG_PRESENTATION = {
    ...
    "CMD_ACK": ("important", "🟢", "前台回执", "前台已显示处理中提示"),
    "CMD_DROP": ("normal", "🟠", "命令过滤", "命令未通过入口过滤"),
    "CMD_REPLY_FAIL": ("critical", "🔴", "前台回执", "命令回执发送失败"),
...
class RadarDB:
    ...
    def list_open_jobs(self, limit: int = 20) -> list[sqlite3.Row]:
        with self._connect() as conn:
            return conn.execute(
                "SELECT id, kind, status, priority, run_after, origin, updated_at, payload_json FROM admin_jobs WHERE status IN ('queued','running') ORDER BY priority ASC, COALESCE(run_after, created_at) ASC, id ASC LIMIT ?",
                (limit,),
            ).fetchall()
