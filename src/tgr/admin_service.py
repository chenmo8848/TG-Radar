...
class AdminApp:
    ...
    async def run(self) -> None:
        ...
        if not worker_session.exists():
            try:
                ...
                raise FileNotFoundError("Missing runtime/sessions/tg_radar_admin_worker.session. Run bootstrap_session.py again.") from exc
    ...
    async def bootstrap_startup_snapshot_if_needed(self) -> None:
        ...
        if not need_bootstrap:
            total_cache = sum(self.db.count_cache_for_folder(row["folder_name"]) for row in rows)
    ...
    async def notify_job_failure(self, job: AdminJob, exc: Exception) -> None:
        reply_to = int(job.payload.get("reply_to") or 0)
        if reply_to:
            await self.edit_message_by_id(reply_to, panel("TR 管理器 · 后台任务执行失败", [section("异常说明", [blockquote_preview(str(exc), 500)])], "<i>详细堆栈已写入 admin.log，可在终端执行 <code>TR logs admin</code> 排查。</i>"))
    ...
    async def send_sync_report(self, sync_report: SyncReport, route_report: RouteReport, automatic: bool = False) -> None:
        ...
        title = "TR 管理器 · 自动同步报告" if automatic else "TR 管理器 · 手动同步报告"
        ...
        active_rows = [f"· <b>{escape(name)}</b> · <code>{count}</code>" for name, count in sorted(sync_report.active.items())]
        if not active_rows:
            active_rows = ["· <i>当前没有可用的分组缓存。</i>"]
        text = panel(title, [section("执行概览", [bullet("结果", status, code=False), bullet("耗时", f"{sync_report.elapsed_seconds:.1f} 秒"), bullet("新分组", len(sync_report.discovered)), bullet("改名", len(sync_report.renamed)), bullet("删除", len(sync_report.deleted))]), section("分组变动", folder_rows), section("自动归纳", route_rows), section("分组缓存规模", active_rows)])
    ...
    async def send_startup_notification(self) -> None:
        ...
        total_cached = sum(self.db.count_cache_for_folder(row["folder_name"]) for row in rows)
        enabled_cached = sum(self.db.count_cache_for_folder(row["folder_name"]) for row in enabled_rows)
        target_map, valid_rules = self.db.build_target_map(self.config.global_alert_channel_id)
        folder_blocks = []
        for row in rows[:12]:
            folder_blocks.append(f"· {escape(row['folder_name'])} · 监听 <code>{'开启' if int(row['enabled']) == 1 else '关闭'}</code> · 群组 <code>{self.db.count_cache_for_folder(row['folder_name'])}</code> · 规则 <code>{self.db.count_rules_for_folder(row['folder_name'])}</code>")
        if not folder_blocks:
            folder_blocks = ["· <i>当前还没有任何分组记录。</i>"]
        await self.plugin_manager.run_healthchecks()
        plugin_summary = [
            bullet("Admin 插件", sum(1 for item in self.plugin_manager.list_plugins('admin') if item.loaded)),
            bullet("Core 插件", sum(1 for item in self.plugin_manager.list_plugins('core') if item.loaded)),
            bullet("加载错误", len([x for x in self.plugin_manager.list_plugins() if x.load_error])),
        ]
        text = panel(
            "TR 管理器 · 启动通知",
            [
    ...
    def render_help_message(self) -> str:
        ...
        sections = [section("帮助说明", ["· 帮助内容按已注册插件实时生成。", "· 轻命令直接回复，重命令进入后台调度。", "· 插件状态请发送 <code>{0}plugins</code>。".format(escape(self.config.cmd_prefix))])]
        for category in sorted(groups):
            sections.append(section(category, groups[category]))
        sections.append(section("规则输入说明", ["· 空格、英文逗号、中文逗号都会被识别为分隔符。", "· 单个正则表达式会按原样使用。", "· 同名规则默认追加，不会直接覆盖旧词。", "· 分组名、规则名、短语关键词如包含空格，请用引号包起来。"]))
        return panel("TR 管理器 · 命令总表", sections, "<i>所有文案与终端管理器口径统一，Telegram 与 Linux 侧统一使用 TR 管理器命名。</i>")
    ...
    def render_sync_message(self, sync_report: SyncReport, route_report: RouteReport) -> str:
        ...
        return panel("TR 管理器 · 同步完成", [section("同步结果", [bullet("变动状态", "发现变动并已更新" if sync_report.has_changes else "数据无变动", code=False), bullet("耗时", f"{sync_report.elapsed_seconds:.1f} 秒"), bullet("新分组", len(sync_report.discovered)), bullet("改名", len(sync_report.renamed)), bullet("删除", len(sync_report.deleted))]), section("分组变动", folder_rows), section("自动归纳", route_rows)], f"<i>如果发现了新分组，记得发送 <code>{escape(self.config.cmd_prefix)}enable 分组名</code> 开启监控。</i>")
    ...
    def render_plugins_message(self) -> str:
        ...
        for kind, title in (("admin", "Admin 插件"), ("core", "Core 插件")):
            ...
            for record in self.plugin_manager.list_plugins(kind):
                state = "已加载" if record.loaded else "加载失败"
        ...
        return panel("TR 管理器 · 插件状态", sections, f"<i>发送 <code>{escape(self.config.cmd_prefix)}pluginreload</code> 可重新加载 Admin 插件命令表。</i>")
